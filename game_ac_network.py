    ...
    def __init__(self,
        ...
        with tf.device(self._device), tf.variable_scope(scope_name) as scope:
            ...
            for i in range(FLAGS.L - 1):
                for j in range(FLAGS.M):
                    if (i == 0):
                        self.W_conv[i, j], self.b_conv[i, j] = self._conv_variable(
                            [kernel_num[i], kernel_num[i], 4, 8]);
                    else:
                        self.W_conv[i, j], self.b_conv[i, j] = self._conv_variable(
                            [kernel_num[i], kernel_num[i], 8, 8]);
            ...
            for s in range(FLAGS.worker_hosts_num):
                for i in range(len(self.geopath_set[0])):
                    for j in range(len(self.geopath_set[0][0])):
                        tf.placeholder(self.geopath_set[s][i, j].dtype, shape=self.geopath_set[s][i, j].get_shape());
                        self.geopath_update_placeholders_set[s][i, j] = tf.placeholder(self.geopath_set[s][i, j].dtype,
                                                                                       shape=self.geopath_set[s][
                                                                                           i, j].get_shape());
                        self.geopath_update_ops_set[s][i, j] = self.geopath_set[s][i, j].assign(
